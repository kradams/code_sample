...
def train_test_split(df, split_col, pct_dev=0.15, pct_test=0.15, seed=3):
    ...
    input: split_col = column on which to split
           pct_dev = percent of split col unique in dev set (default = 0.15)
           pct_test = percent of split col unique in test set (default = 0.15)
...
def plot_facets(groups_list, elem_name):
    ...
    :param groups_list: list of dictionaries of the form {'name': '<name for data>', 'table': <data frame>}
...
def get_target_ahead(df, patient_id, time_col, label_col, time_ahead):
    ...
    :return: data frame containing columns for the label at the target prediction horizon
    '''
...
def filter_rows(df, time_idx, label_col, label_col_ahead, group_name, pred_hours_ahead):
    ...
    :param df: pandas data frame, data frame for filtering
...
def impute_carryforward_group_simple(df, group_name, group_metric, metric, fcols, missing_indicator=True):
    ...
    :param df: data frame with values for imputation
    ...
    :param metric: metric to use for imputation across the entire data frame
    ...
    :return: the data frame with <col>_fill and, if desired, <col>_missing columns for all columns in fcol
    '''
